...
automata_file = "automata_lemmaPos2concepts.txt"
file_data = []
concept_dict = {}
concepts_data = []
test_data = []
with open(train_file) as f, open(train_lemma_pos) as f1:
    ...
    for line_train, line_feats in zip(tmp, tmp1):
        if line_train != '':
            ...
            file_data.append((word_tag_lemma[2], word_tag_lemma[1], word_concepts[1]))
...
with open(automata_file, "w") as w:
    for item in data:
        ...
        print("0" + "\t0\t" + str(item[0]+"$"+item[1]) + "\t" + str(item[2]) + "\t" + str(prob), file=w)
...
with open(test_file) as f:
    ...
    for line in tmp:
        if line != '':
            ...
            output_string = output_string + str(lemma+"$"+pos) + "\t"
        else:
